...
if True == True:  # Shutdown trigger
    ...
    try:
        ...
        logger.warn(
            f"Something is listening on port {port}; check open connection and restart JDH."
        )
...
def cli_infer(com):
    ...
    print("JDH Infer-CLI: Starting the inference...")
    vc_data = vc.get_vc(model_name, protection_amnt, protect1)
    print(vc_data)
    print("JDH Infer-CLI: Performing inference...")
    ...
    if "Success." in conversion_data[0]:
        print("JDH Infer-CLI: Inference succeeded.")
    else:
        print("JDH Infer-CLI: Inference failed. Here's the traceback: ")
...
def cli_extract_feature(com):
    ...
    print("JDH-CLI: Extract Feature Has Pitch: " + str(has_pitch_guidance))
    print("JDH-CLI: Extract Feature Version: " + str(version))
    print("JDH Feature Extraction: Starting...")
    ...
    execute_generator_function(generator)
    print("JDH Feature Extraction: Finished")
...
def cli_train_feature(com):
    ...
    model_name = com[0]
    version = com[1]
    print("JDH Train Feature Index-CLI: Training... Please wait")
    generator = train_index(model_name, version)
    execute_generator_function(generator)
    print("JDH Train Feature Index-CLI: Done!")
...
def cli_extract_model(com):
    ...
    if extract_small_model_process == "Success.":
        print("JDH Extract Small Model: Success!")
    else:
        print(str(extract_small_model_process))
        print("JDH Extract Small Model: Failed!")
...
def GradioSetup():
    ...
    with gr.Blocks(theme=my_applio, title="JDH") as app:
        gr.HTML("<h1> JDH </h1>")
        with gr.Tabs():
